...
public:
    ...
    std::string string;                  // дл€ линейно-скобочной записи.

    Tree(const int v, Tree* p){      // алгоритм, который позвол€ет положить значение в виде листа одного из узлов
        value = v;
        parent = p;
    }
    Tree(){}
    ...
    void add(const int v){             // ћетод добавлени€ значени€ в дерево
        ...
        else{                             // иначе добавл€ем в левое поддерево, или создаем это поддерево если его нет
            if (right == nullptr){
            if (left == nullptr){
...
//удалени€ элемента делитс€ на несколько случаев:у узла нет дочерних узлов; у узла есть левый дочерних узлов;у узла есть правый дочерних узлов; у узла есть оба ребЄнка.
//¬ первом просто удал€ем узел.¬ случае 2 и 3 замен€ем удал€емый узел на его потомка,на этом удаление заканчиваетс€.
//—лучай 4, дл€ этого необходимо найти в правом поддереве минимальный элемент и переместить его на место удал€емого узла.
    void remove(int value){
        Tree* item = this->Search(value);
        if (item == nullptr) return;      // если такого элемента нет удал€ть не нужно
        Tree* currentTree = nullptr;
        // удаление корн€
        if (this == item){
            if (item->right != nullptr){    // если есть правое поддерево, тогда ищем в нЄм самый наименьший элемент(левый)
            ...
            else if (item->left != nullptr){   // если есть только левое дерево, тогда ищем в нЄм самый наименьший элемент(правый)
